...
const login = defaultRequire(`${process.cwd()}/fb-chat-api2`);
...
let subTitle = `Meow V3@${currentVersion} - A simple bot chat messenger using personal account`;
const subTitleArray = [];
...
const author = "Created by Nazrul with ♡";
const srcUrl = "Source code: https://github.com/nazrul4x/Meow-v3";
...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        await require("../autoSyncAll.js")(globalData);
                        ...
                        global.lastMessageTime = Date.now();
                        const WATCHDOG_CHECK_INTERVAL = 5 * 60 * 1000;
                        const WATCHDOG_TIMEOUT = 50 * 60 * 1000;
                        
                        const watchdogInterval = setInterval(async function() {
                                const timeSinceLastMessage = Date.now() - (global.lastMessageTime || Date.now());
                                if (timeSinceLastMessage > WATCHDOG_TIMEOUT) {
                                        log.warn("WATCHDOG", `No messages received for ${Math.round(timeSinceLastMessage / 60000)} minutes, restarting connection...`);
                                        try {
                                                await stopListening();
                                                await sleep(3000);
                                                global.GoatBot.Listening = api.listenMqtt(createCallBackListen());
                                                global.lastMessageTime = Date.now();
                                                log.info("WATCHDOG", "Connection restarted successfully");
                                        } catch (e) {
                                                log.err("WATCHDOG", "Failed to restart connection, restarting bot...", e);
                                                process.exit(2);
                                        }
                                }
                        }, WATCHDOG_CHECK_INTERVAL);
                        global.watchdogInterval = watchdogInterval;
                        log.info("WATCHDOG", "Connection watchdog enabled - will restart if no messages for 10 minutes");

                        const FB_DTSG_REFRESH_INTERVAL = 4 * 60 * 60 * 1000;
                        const fbDtsgRefreshInterval = setInterval(async function() {
                                try {
                                        await api.refreshFb_dtsg();
                                        log.info("FB_DTSG", "Session tokens refreshed successfully");
                                } catch (e) {
                                        log.warn("FB_DTSG", "Failed to refresh session tokens", e);
                                }
                        }, FB_DTSG_REFRESH_INTERVAL);
                        global.fbDtsgRefreshInterval = fbDtsgRefreshInterval;
                        log.info("FB_DTSG", "Auto session refresh enabled - every 4 hours");
                });
        })(appState);
